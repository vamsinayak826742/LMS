...
function Return() {
    ...
    const returnBook = async (transactionId, borrowerId, bookId, due) => {
        try {
            console.log(`Making PUT request to: ${API_URL}/api/transactions/update-transaction/${transactionId}`);
            console.log("Payload:", {
            ...
            console.log("Error in returnBook:", err);
    ...
    const convertToIssue = async (transactionId) => {
        try {
            ...
            console.log("Error in convertToIssue:", err);
            alert("Error: Unable to issue book");
        }
    };
